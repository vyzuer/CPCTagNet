...
models = {0:'tagnet_pairwiseranking', 1:'contagnet', 2:'uctagnet', 3:'cuctagnet'}
...
def check_accuracy(net, k=5, context=1):
    ...
    fname = dump_dir + 'prediction_val_warp_' + str(k) + '_' + str(context) + '.tsv'
    fp = open(fname, 'w')
    num_asamples = 0
    num_batches = n_samples/batch_size
    acc = 0.0
...
def master(model_dir, context, viz = False):
    ...
    model_weights = None
    model_weights = caffe_root + 'models/' + models[context] + '/alexnet_' + models[context] + '_1_iter_400000.caffemodel'
    # model_weights = caffe_root + 'models/' + models[context] + '/alexnet_' + models[context] + '_0_iter_77.caffemodel'
    # if context:
