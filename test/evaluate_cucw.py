...
models = {0:'tagnet_warp', 1:'contagnet', 2:'uctagnet', 3:'cuctagnet', 4:'cuctagnet_warp'}
prediction_layers = {0:'fc8_tag', 1:'fc9_flickr_con', 2:'fc10_flickr_con', 3:'fc10_flickr_con', 4:'fc10_flickr_con'}
...
def check_accuracy(net, k=5, context=1):
    ...
    fname = dump_dir + 'prediction_val_cucwarp_' + str(k) + '_' + str(context) + '.tsv'
    fp = open(fname, 'w')
    num_asamples = 0
    num_batches = n_samples/batch_size
    acc = 0.0
...
def master(model_dir, context, viz = False):
    ...
    model_weights = None
    model_weights = caffe_root + 'models/' + models[context] + '/alexnet_' + models[context] + '_0_iter_400000.caffemodel'
    # if context:
    ...
    fname = dump_dir + 'results_warp_' + str(context) + '.list'
    fp = open(fname, 'w')
    for k in (1, 3, 5, 10):
    # for k in ([5]):
